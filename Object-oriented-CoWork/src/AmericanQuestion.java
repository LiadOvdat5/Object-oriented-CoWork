...
public class AmericanQuestion extends Question {
	...
	private ArrayList<Answer> allAnswers = new ArrayList<Answer>();
	private ArrayList<Answer> rightAnswers = new ArrayList<Answer>();

	public AmericanQuestion(String content, ArrayList<Answer> answers) throws GeneralSystemException { // American Q
																										// C'tor
		super(content);
		...
		for (int i = 0; i < allAnswers.size(); i++) { // check number of right answers
			if (allAnswers.get(i).getIsRight()) {
				this.rightAnswers.add(allAnswers.get(i)); // Right answers will be same answers from all answers array
		...
		if (moreThanOne) // if more than one answer is right than make all answers wrong except
							// "moreThan1" ans
	...
	public String toString() {// To String - print;
		...
		return sBuffer.toString();
		/*
		 * for(Answer ans: allAnswers) sBuffer.append(ans.toString() + "\n");
		 * sBuffer.append("\n"); return sBuffer.toString();
		 */
	}
	...
	public String saveQuestion() {
		...
		for (Answer ans : allAnswers)
			sBuffer.append((allAnswers.indexOf(ans) + 1) + ")" + ans.getContent() + "\n");
	...
	public String saveAnswer() {
		...
		for (int i = 0; i < this.allAnswers.size(); i++) {
			sBuffer.append(
					(i + 1) + ")" + this.allAnswers.get(i).content + " | " + this.allAnswers.get(i).isRight + "\n");
		}
		return sBuffer.toString();
	}
	...
	public int answerLength() {
		...
		/*
		 * for(int i = 0; i < this.allAnswers.size(); i++) if(allAnswers.get(i) != null)
